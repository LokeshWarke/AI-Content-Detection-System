...
# Enhanced detection (optional but recommended for better accuracy)
try:
    from enhanced_detector import EnhancedAIDetector
    ENHANCED_DETECTOR = None  # Will be initialized lazily
    USE_ENHANCED = True
except ImportError:
    USE_ENHANCED = False
...
<style>
    ...
    .main .block-container h1{
        ...
        text-shadow: 0 2px 12px rgba(15,23,42,0.08) !important;
    }
    .main .block-container h2{
    ...
    .final-result-card{
        ...
        padding: 1.75rem;
        border-radius: 16px;
        background: linear-gradient(135deg, rgba(255,107,107,0.12), rgba(99,102,241,0.12));
    ...
    .final-result-metrics .metric-card{
        flex: 1;
        min-width: 180px;
    }
</style>
...
class TextAnalyzer:
    def __init__(self):
        ...
        if USE_ENHANCED:
            try:
                self.enhanced_detector = EnhancedAIDetector(method="transformer")
                st.sidebar.success("✅ Enhanced Detector (Transformer) Active")
            except Exception as e:
                st.sidebar.warning(f"⚠️ Enhanced detector unavailable: {str(e)[:50]}")
                self.enhanced_detector = None
    ...
    def get_ml_prediction(self, text: str) -> float:
        ...
        if self.enhanced_detector:
            try:
                result = self.enhanced_detector.predict(text)
...
# Show enhanced detector status in sidebar
if not USE_ENHANCED:
    with st.sidebar:
        st.info("💡 Install transformers for better accuracy:\n`pip install transformers torch`")
...
if analyze_button:
    if text_input or uploaded_file:
        ...
        with st.spinner("Analyzing content with advanced algorithms..."):
            ...
            # Determine score based on method for downstream displays
            ...
            if ai_probability >= 65:
                ...
                verdict_color = "#EF553B"
                verdict_reason = "AI indicators crossed the 65% alert threshold."
                summary_block_html = "<div class='stError'>High confidence that this content was AI-generated.</div>"
                summary_text = "The text exhibits consistent structure, stable vocabulary, and low burstiness—patterns frequently seen in AI outputs."
            elif human_probability >= 65:
                verdict_label = "Likely Human-written"
                verdict_color = "#00CC96"
                verdict_reason = "Human probability remains safely above 65%."
                summary_block_html = "<div class='stSuccess'>High confidence that this content was written by a human.</div>"
                summary_text = "Natural sentence variety, mixed vocabulary, and higher perplexity align with typical human writing."
            else:
                verdict_label = "Inconclusive / Mixed"
                verdict_color = "#FFA500"
                verdict_reason = "Signals conflict; manual review recommended."
                summary_block_html = "<div class='stWarning'>Moderate confidence - mixed signals detected.</div>"
                summary_text = "The analysis reports overlapping human and AI traits. Gathering more context or additional samples can improve certainty."

            # Display results with interactive tabs
            st.markdown("## Analysis Results")
            st.caption("Navigate between overview, deep-dive metrics, and raw text stats.")
            overview_tab, detail_tab, stats_tab = st.tabs(["Overview", "Deep Dive", "Text Stats"])
            ...
            with overview_tab:
                ...
                with col2:
                    ...
                    <div class="metric-card">
                        ...
                        <p style="margin:0;color:#475569;font-size:0.9rem;">{verdict_reason}</p>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    <div class="metric-card">
                        <h4>Verdict</h4>
                        <h2 style="color: {verdict_color};">{verdict_label}</h2>
                    </div>
                    """, unsafe_allow_html=True)

                summary_table = pd.DataFrame([
                    {"Metric": "Human Probability", "Value": f"{human_probability:.1f}%"},
                    {"Metric": "AI Probability", "Value": f"{ai_probability:.1f}%"},
                    {"Metric": "Confidence Level", "Value": confidence},
                    {"Metric": "Verdict", "Value": verdict_label},
                ])
                st.dataframe(summary_table.set_index("Metric"), use_container_width=True)
            ...
            with detail_tab:
                if show_explanation:
                    if detection_method in ["Stylometric Analysis", "Combined Analysis"] and "stylometric" in results:
                        ...
                        feature_values = [float(np.clip(v, -1e6, 1e6)) for v in features.values()]
                        fig = go.Figure(data=[
                            go.Bar(
                                x=feature_names,
                                y=feature_values,
                        ...
                        st.dataframe(feature_table.set_index("Feature"), use_container_width=True)
                    ...
                    if detection_method in ["Perplexity Analysis", "Combined Analysis"] and "perplexity" in results:
                        ...
                        perplexity_table = pd.DataFrame([
                            {"Metric": "Normalized Score", "Value": f"{perplexity_score*100:.1f}%"},
                            {"Metric": "Raw Perplexity", "Value": f"{details.get('ppl', 0):.1f}"},
                            {"Metric": "Burstiness", "Value": f"{details.get('burstiness', 0):.3f}"}
                        ])
                        st.dataframe(perplexity_table.set_index("Metric"), use_container_width=True)
                    ...
                    if detection_method in ["ML Classification", "Combined Analysis"] and "ml_score" in results:
                        ...
                        ml_table = pd.DataFrame([
                            {"Metric": "Human Probability", "Value": f"{human_ml_prob:.1f}%"},
                            {"Metric": "AI Probability", "Value": f"{ai_ml_prob:.1f}%"},
                            {"Metric": "Threshold", "Value": f"{analyzer.detector.threshold:.2f}"}
                        ])
                        if "ml_pred" in results:
                            ml_table = pd.concat([ml_table, pd.DataFrame([{"Metric": "Model Class", "Value": "Human" if results["ml_pred"] == 0 else "AI"}])])
                        st.dataframe(ml_table.set_index("Metric"), use_container_width=True)
                else:
                    st.info("Enable 'Show Detailed Explanation' in the sidebar to view feature-level insights.")
            ...
            with stats_tab:
                ...
                stats_table = pd.DataFrame([
                    {"Metric": "Word Count", "Value": word_count},
                    {"Metric": "Character Count", "Value": char_count},
                    {"Metric": "Sentence Count", "Value": sentence_count}
                ])
                st.table(stats_table.set_index("Metric"))

            # Final summary section anchored at the bottom of the results
            final_container = st.container()
            final_container.markdown("## Final Result")
            final_container.markdown(summary_block_html, unsafe_allow_html=True)
            final_container.markdown(f"""
            <div class="final-result-card">
                <div class="final-result-pill" style="background-color: {verdict_color}1A; color: {verdict_color};">
                    Final Verdict
                </div>
                <h3>{verdict_label}</h3>
                <p style="margin-bottom: 1rem;">{verdict_reason}</p>
            ...
            """, unsafe_allow_html=True)
            final_container.markdown(summary_text)
